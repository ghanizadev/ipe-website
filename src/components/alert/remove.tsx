...
export default function RemoveAlert() {
  ...
  const handleAction = (accepted: boolean, dismissed = false) => {
    return () => {
      if (!props) return;

      const event = new CustomEvent(props.callbackName, {
        detail: { accepted, dismissed },
      });
  ...
  useEffect(() => {
    window.addEventListener('alert.remove', handleInvocation);

    return () => {
      window.removeEventListener('alert.remove', handleInvocation);
    };
  }, []);
